...
!            do k=1,levs
!              do i=1,im
!                Diag%dtend(i,k,6) = dtend_loc(i,k,6)
!                Diag%dtend(i,k,7) = dtend_loc(i,k,7)
!                Diag%dtend(i,k,8) = dtend_loc(i,k,8)
!                Diag%dtend(i,k,9) = dtend_loc(i,k,9)
!              enddo
!            enddo
!          endif
        else
...
!             Diag%dtend(i,k,2) = Diag%dtend(i,k,2) + (Stateout%gq0(i,k,1)-dqdt(i,k,1)) * frain
...
!               Diag%dtend(i,k,3) = Diag%dtend(i,k,3) + (Stateout%gq0(i,k,1)-dqdt(i,k,1)) * frain
              enddo
            enddo
          endif
        endif   ! end if_lssav
...
!             Diag%dtend(i,k,4) = Diag%dtend(i,k,4) + (Stateout%gq0(i,k,1)-dqdt(i,k,1)) * frain
            enddo
          enddo
        endif
      endif
