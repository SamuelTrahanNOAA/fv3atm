...
!   allocate (temp2d(isc:iec,jsc:jec,nsfcprop2d+Model%ntot3d+Model%nctp))
   ...
   do j=jsc,jec
     do i=isc,iec
       ...
       if (Model%lsm == Model%lsm_noahmp) then
        temp2d(i,j,idx_opt+1)    = GFS_Data(nb)%Sfcprop%snowxy(ix)
        temp2d(i,j,idx_opt+2)  = GFS_Data(nb)%Sfcprop%tvxy(ix)
        temp2d(i,j,idx_opt+3)  = GFS_Data(nb)%Sfcprop%tgxy(ix)
        temp2d(i,j,idx_opt+4)  = GFS_Data(nb)%Sfcprop%canicexy(ix)
        temp2d(i,j,idx_opt+5)  = GFS_Data(nb)%Sfcprop%canliqxy(ix)
        temp2d(i,j,idx_opt+6)  = GFS_Data(nb)%Sfcprop%eahxy(ix)
        temp2d(i,j,idx_opt+7)  = GFS_Data(nb)%Sfcprop%tahxy(ix)
        temp2d(i,j,idx_opt+8)  = GFS_Data(nb)%Sfcprop%cmxy(ix)
        temp2d(i,j,idx_opt+9)  = GFS_Data(nb)%Sfcprop%chxy(ix)
        temp2d(i,j,idx_opt+10)  = GFS_Data(nb)%Sfcprop%fwetxy(ix)
        temp2d(i,j,idx_opt+11) = GFS_Data(nb)%Sfcprop%sneqvoxy(ix)
        temp2d(i,j,idx_opt+12) = GFS_Data(nb)%Sfcprop%alboldxy(ix)
        temp2d(i,j,idx_opt+13) = GFS_Data(nb)%Sfcprop%qsnowxy(ix)
        temp2d(i,j,idx_opt+14) = GFS_Data(nb)%Sfcprop%wslakexy(ix)
        temp2d(i,j,idx_opt+15) = GFS_Data(nb)%Sfcprop%zwtxy(ix)
        temp2d(i,j,idx_opt+16) = GFS_Data(nb)%Sfcprop%waxy(ix)
        temp2d(i,j,idx_opt+17) = GFS_Data(nb)%Sfcprop%wtxy(ix)
        temp2d(i,j,idx_opt+18) = GFS_Data(nb)%Sfcprop%lfmassxy(ix)
        temp2d(i,j,idx_opt+19) = GFS_Data(nb)%Sfcprop%rtmassxy(ix)
        temp2d(i,j,idx_opt+20) = GFS_Data(nb)%Sfcprop%stmassxy(ix)
        temp2d(i,j,idx_opt+21) = GFS_Data(nb)%Sfcprop%woodxy(ix)
        temp2d(i,j,idx_opt+22) = GFS_Data(nb)%Sfcprop%stblcpxy(ix)
        temp2d(i,j,idx_opt+23) = GFS_Data(nb)%Sfcprop%fastcpxy(ix)
        temp2d(i,j,idx_opt+24) = GFS_Data(nb)%Sfcprop%xsaixy(ix)
        temp2d(i,j,idx_opt+25) = GFS_Data(nb)%Sfcprop%xlaixy(ix)
        temp2d(i,j,idx_opt+26) = GFS_Data(nb)%Sfcprop%taussxy(ix)
        temp2d(i,j,idx_opt+27) = GFS_Data(nb)%Sfcprop%smcwtdxy(ix)
        temp2d(i,j,idx_opt+28) = GFS_Data(nb)%Sfcprop%deeprechxy(ix)
        temp2d(i,j,idx_opt+29) = GFS_Data(nb)%Sfcprop%rechxy(ix)

        temp2d(i,j,idx_opt+30) = GFS_Data(nb)%Sfcprop%snicexy(ix,-2)
        temp2d(i,j,idx_opt+31) = GFS_Data(nb)%Sfcprop%snicexy(ix,-1)
        temp2d(i,j,idx_opt+32) = GFS_Data(nb)%Sfcprop%snicexy(ix,0)
        temp2d(i,j,idx_opt+33) = GFS_Data(nb)%Sfcprop%snliqxy(ix,-2)
        temp2d(i,j,idx_opt+34) = GFS_Data(nb)%Sfcprop%snliqxy(ix,-1)
        temp2d(i,j,idx_opt+35) = GFS_Data(nb)%Sfcprop%snliqxy(ix,0)
        temp2d(i,j,idx_opt+36) = GFS_Data(nb)%Sfcprop%tsnoxy(ix,-2)
        temp2d(i,j,idx_opt+37) = GFS_Data(nb)%Sfcprop%tsnoxy(ix,-1)
        temp2d(i,j,idx_opt+38) = GFS_Data(nb)%Sfcprop%tsnoxy(ix,0)
        temp2d(i,j,idx_opt+39) = GFS_Data(nb)%Sfcprop%smoiseq(ix,1)
        temp2d(i,j,idx_opt+40) = GFS_Data(nb)%Sfcprop%smoiseq(ix,2)
        temp2d(i,j,idx_opt+41) = GFS_Data(nb)%Sfcprop%smoiseq(ix,3)
        temp2d(i,j,idx_opt+42) = GFS_Data(nb)%Sfcprop%smoiseq(ix,4)
        temp2d(i,j,idx_opt+43) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,-2)
        temp2d(i,j,idx_opt+44) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,-1)
        temp2d(i,j,idx_opt+45) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,0)
        temp2d(i,j,idx_opt+46) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,1)
        temp2d(i,j,idx_opt+47) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,2)
        temp2d(i,j,idx_opt+48) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,3)
        temp2d(i,j,idx_opt+49) = GFS_Data(nb)%Sfcprop%zsnsoxy(ix,4)
        idx_opt = idx_opt + 49
       elseif (Model%lsm == Model%lsm_ruc) then
        temp2d(i,j,idx_opt+1)    = GFS_Data(nb)%Sfcprop%wetness(ix)
        temp2d(i,j,idx_opt+2)  = GFS_Data(nb)%Sfcprop%clw_surf_land(ix)
        temp2d(i,j,idx_opt+3)  = GFS_Data(nb)%Sfcprop%clw_surf_ice(ix)
        temp2d(i,j,idx_opt+4)  = GFS_Data(nb)%Sfcprop%qwv_surf_land(ix)
        temp2d(i,j,idx_opt+5)  = GFS_Data(nb)%Sfcprop%qwv_surf_ice(ix)
        temp2d(i,j,idx_opt+6)  = GFS_Data(nb)%Sfcprop%tsnow_land(ix)
        temp2d(i,j,idx_opt+7)  = GFS_Data(nb)%Sfcprop%tsnow_ice(ix)
        temp2d(i,j,idx_opt+8)  = GFS_Data(nb)%Sfcprop%snowfallac_land(ix)
        temp2d(i,j,idx_opt+9)  = GFS_Data(nb)%Sfcprop%snowfallac_ice(ix)
       ...
       if (Model%nstf_name(1) > 0) then
         temp2d(i,j,idx_opt+1 ) = GFS_Data(nb)%Sfcprop%tref(ix)
         temp2d(i,j,idx_opt+2) = GFS_Data(nb)%Sfcprop%z_c(ix)
         temp2d(i,j,idx_opt+3) = GFS_Data(nb)%Sfcprop%c_0(ix)
         temp2d(i,j,idx_opt+4) = GFS_Data(nb)%Sfcprop%c_d(ix)
         temp2d(i,j,idx_opt+5) = GFS_Data(nb)%Sfcprop%w_0(ix)
         temp2d(i,j,idx_opt+6) = GFS_Data(nb)%Sfcprop%w_d(ix)
         temp2d(i,j,idx_opt+7) = GFS_Data(nb)%Sfcprop%xt(ix)
         temp2d(i,j,idx_opt+8) = GFS_Data(nb)%Sfcprop%xs(ix)
         temp2d(i,j,idx_opt+9) = GFS_Data(nb)%Sfcprop%xu(ix)
         temp2d(i,j,idx_opt+10) = GFS_Data(nb)%Sfcprop%xz(ix)
         temp2d(i,j,idx_opt+11) = GFS_Data(nb)%Sfcprop%zm(ix)
         temp2d(i,j,idx_opt+12) = GFS_Data(nb)%Sfcprop%xtts(ix)
         temp2d(i,j,idx_opt+13) = GFS_Data(nb)%Sfcprop%xzts(ix)
         temp2d(i,j,idx_opt+14) = GFS_Data(nb)%Sfcprop%ifd(ix)
         temp2d(i,j,idx_opt+15) = GFS_Data(nb)%Sfcprop%dt_cool(ix)
         temp2d(i,j,idx_opt+16) = GFS_Data(nb)%Sfcprop%qrain(ix)
         idx_opt = idx_opt + 16
       endif
!
        if (Model%lkm > 0 ) then
...
!       Sfcprop(nb)%hprim(ix)     = oro_var2(i,j,1)
        ...
        if (Model%lkm > 0 ) then                    !YWu
           Sfcprop(nb)%lakefrac(ix)  = oro_var2(i,j,18) !lake frac [0:1]
           Sfcprop(nb)%lakedepth(ix) = oro_var2(i,j,19) !lake depth [m]   
        endif
...
!          write(0,*) 'lakefrac(ix)= ',Sfcprop(nb)%lakefrac(ix),ix
!        endif
        Sfcprop(nb)%oceanfrac(ix)= one - Sfcprop(nb)%landfrac(ix) - Sfcprop(nb)%lakefrac(ix)
!        if(Sfcprop(nb)%fice(ix)<Sfcprop(nb)%oceanfrac(ix)) then
!           write(0,*) ' Sfcprop(nb)%fice(ix)= ',Sfcprop(nb)%fice(ix), Sfcprop(nb)%oceanfrac(ix), &
!     &                  Sfcprop(nb)%landfrac(ix), Sfcprop(nb)%lakefrac(ix), ix                          
!           Sfcprop(nb)%fice(ix) = 0.0
!           Sfcprop(nb)%oceanfrac(ix) = 1.0
!        endif
...
!                  Sfcprop(nb)%oceanfrac(ix) = one
          ...
          if (Sfcprop(nb)%landfrac(ix) > -999.0_r8) then    !< 0??? ocean/lake, ice        
            if (Sfcprop(nb)%lakefrac(ix) > zero) then
!              Sfcprop(nb)%oceanfrac(ix) = zero
              Sfcprop(nb)%landfrac(ix)  = zero
!              Sfcprop(nb)%lakefrac(ix)  = one
...
!       do num = nvar2m+nvar2o+nvar2r+nvar2mp+1,nvar2m+nvar2o+nvar2r+nvar2mp+nvar2l 
          var2_p => sfc_var2(:,:,num)
          call register_restart_field(Sfc_restart, sfc_name2(num), var2_p,dimensions=(/'xaxis_1', 'yaxis_1', 'Time   '/),&
                                     &is_optional=.not.mand)
       enddo
